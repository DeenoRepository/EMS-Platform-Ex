...
## Уточнения стабилизации ядра (2026-09-07)

- Login и bootstrap используют атомарный get-or-create по `(directory_id, object_guid)`; найденная запись блокируется `FOR UPDATE` внутри транзакции до чтения ролей и создания сессии.
- Перевод и bootstrap сериализуются на строке сотрудника; bootstrap существующего сотрудника отзывает его сессии в той же транзакции, что назначение и аудит.
