...
- Статус: **Approved**
- Версия решения: 1.0.1
- Владелец решения: владелец проекта
- Утверждено: владельцем проекта (2026-09-09)
- Изменено: 2026-09-09, версия 1.0.1 — версия `ldapts` синхронизирована с ADR-0004 1.1.0 (`8.2.0`); целевой runtime Node.js 20 LTS не изменен
...
2. LDAP adapter использует `ldapts` 8.2.0 только на сервере, LDAPS или StartTLS с обязательной проверкой сертификата доверенного CA. Connect, bind и search ограничиваются конфигурируемым timeout с baseline 5 секунд. LDAP filter values экранируются, bind secret и пароль пользователя не логируются.
