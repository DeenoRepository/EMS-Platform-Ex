...
| `ldapts` | `9.0.0` | MIT | server-only LDAP/LDAPS adapter |

Версии и лицензии проверены по npm metadata 2026-09-09. Установка зависимостей выполняется на следующем этапе; этот ADR не изменяет `package.json` или lockfile.
